...
1. Mengapa peneliti psikologi perlu memiliki literasi data?
2. Apa itu data?
3. Jenis-jenis data
4. Kerangka kerja *Data Pipeline*
5. Apa itu literasi data?
6. Dataset yang akan kita pakai sepanjang semester
...
::: {aside}
Sumber: [Open Knowledge Foundation (2021)](libs/okf.pdf)
:::
...
- Satu dataset biasanya berisi banyak **variabel** (kolom) yang diukur pada banyak **unit analisis/observasi** (baris) — kita akan bahas ini lebih detail di Pertemuan 2.
...
- Contoh: *reaction time* (ms), durasi pengisian kuesioner, skor total skala yang dihitung dari rata-rata (mis. 3,4).
- Skor rata-rata 10 item Likert 1–5 sebenarnya diskrit (hanya kelipatan 0,1), tetapi dalam praktik biasanya **diperlakukan** sebagai *continuous*.
...
Catatan: kualitatif ≠ tidak terstruktur, dan kuantitatif ≠ selalu berupa data interval/rasio. Meski begitu, kedua pembedaan ini saling berkaitan: data kategorikal termasuk data kualitatif, sedangkan data numerik termasuk data kuantitatif.
...
| Skor total skala kepribadian | Numerik (diskrit, biasanya diperlakukan *continuous*) | Kuantitatif |
...
- Kerangka ini akan menjadi ***workflow*** kita sampai Pertemuan 7.
:::
...
- Tema umum (mis. "kesejahteraan psikologis mahasiswa") diturunkan jadi pertanyaan spesifik (mis. "apakah durasi penggunaan media sosial berkaitan dengan kesejahteraan psikologis mahasiswa tingkat akhir?").
...
::: {.incremental}
- Kemampuan statistik terutama dibutuhkan di **satu tahap** (*Analyze*) dari tujuh tahap *Data Pipeline*, meski tahap *Verify* dan *Clean* juga memakai statistik sederhana.
...
- Pertemuan 2–7 memakai **satu dataset yang sama** dari awal sampai akhir *Data Pipeline*: tabel mentah → bersih → dianalisis → divisualisasikan.
...
- Berisi respons terhadap 50 item kepribadian (10 item untuk masing-masing dari lima dimensi *Big Five*: **E**xtraversion, **N**euroticism, **A**greeableness, **C**onscientiousness, **O**penness) plus data demografis (usia, gender, dsb.) dan kode negara yang ditentukan otomatis dari lokasi teknis (IP) partisipan, bukan diisi sendiri.
:::
...
- **Tidak sempurna** — dikumpulkan daring secara terbuka, sehingga ada data yang perlu diverifikasi dan dibersihkan (Pertemuan 3–4): usia yang tidak masuk akal (mis. tahun lahir atau 999999999), kode negara yang tidak valid (`(nu`, `A1`, `A2`), dan nilai `0` yang berarti tidak dijawab (*missed*).
...
- Literasi data mencakup keseluruhan alur ini, plus *awareness* soal aspek etisnya, jadi sangat berbeda dengan kemampuan statistik.
