...
::: {.callout-important}
Pertanyaan pertama **bukan** *"bagaimana cara memvisualisasikan data ini?"* — melainkan **"apa yang ingin saya buat audiens saya lakukan/pahami dari proyek ini?"**
:::

::: {.incremental}
* Parameter kunci penyajian data adalah **audiens** dan **pesan** — bukan jenis grafik.
* Bagaimana Anda menyajikan hasil lewat radio? Ke audiens dengan gangguan penglihatan? Ke komunitas yang tidak familier dengan grafik? Dalam semua kasus itu, grafik saja tidak cukup.
* Itulah kenapa judul pertemuan ini "**Menyajikan** Data", bukan "**Memvisualisasikan** Data" — visualisasi hanyalah salah satu alat, bukan tujuan itu sendiri.
:::
...
::: {.callout-note}
Ingat kapsul UAS mata kuliah ini: naskah *narrative review* **plus infografis** pendukung. Perbedaan ini akan langsung relevan saat itu.
:::
...
## Kategorikal vs. kontinu (sekali lagi)

Ingat Pertemuan 1: perbedaan data kategorikal dan numerik sekarang menentukan **pilihan grafik** Anda.

::: {.incremental}
* **Data kategorikal** (mis. gender, kelompok skor) — cocok untuk *bar chart*, *pie chart*. Tapi grafik butuh **minimal satu variabel kontinu** untuk dipasangkan (mis. rata-rata skor per kategori) — tidak mungkin membuat grafik hanya dari variabel kategorikal saja.
* **Data kontinu** (mis. usia, skor skala) — bisa dipasangkan di kedua sumbu, seperti pada *scatterplot* atau histogram.
* Data kontinu **bisa** ditampilkan sebagai kategorikal (usia dikelompokkan jadi rentang) — tapi tidak sebaliknya.
:::
...
## Ragam tugas visualisasi dan grafiknya
...
::: {.callout-tip}
Tidak ada daftar tugas visualisasi yang baku dan lengkap — satu grafik bisa melayani lebih dari satu tugas sekaligus. Yang penting: **rumuskan tugasnya dulu**, baru pilih grafiknya — bukan sebaliknya.
:::
...
Kita hitung sungguhan skor *Extraversion* (setelah item *reverse-scored* dibalik, seperti prosedur Pertemuan 4–5) dari **19.511 partisipan** dalam dataset kita, dipilah berdasarkan gender.
...
## Sebelum: melanggar hampir semua prinsip
...
## Sesudah: enam prinsip diterapkan
...
Dengan `n` sebesar ini, CI kedua kelompok **tidak tumpang tindih** — jadi secara statistik perbedaannya "nyata". Tapi besarnya cuma ~0,10 poin pada skala 1–5. Judul grafik sengaja memakai *hedging language* ("sedikit lebih tinggi"), bukan klaim besar — inilah cara **presenting** dan **hedging** bekerja bersama.
:::

## Angka di balik grafiknya

Grafik itu bagus untuk audiens umum — tapi naskah akademik (Pertemuan 6, gaya APA) tetap butuh tabel angka lengkap:
...
Gunakan kedua grafik di atas (atau contoh lain dari jurnal/media yang dibawa dosen). Terapkan enam prinsip *storytelling with data* untuk mengkritiknya:
...
::: {.incremental}
* Mulai dari **audiens dan pesan**, baru pilih visualisasi — bukan sebaliknya.
...
## 🧩 Aktivitas mandiri: rencana visualisasi UTS

::: {.callout-warning}
### Kerjakan sendiri, kumpulkan sebagai persiapan UTS
Merujuk pada dataset yang Anda rencanakan untuk proyek UTS (Pertemuan 3 dan 5):

1. Rumuskan **satu kalimat "saya ingin menunjukkan..."** untuk visualisasi UTS Anda.
2. Tentukan **jenis grafik** yang paling sesuai, dan jelaskan alasannya lewat enam prinsip *storytelling with data*.
3. Sebutkan **alat** yang akan Anda pakai untuk membuatnya.
:::

::: notes
Ini menjembatani langsung ke UTS Pertemuan 8 — pastikan mahasiswa memahami bahwa hasil aktivitas ini bisa langsung dipakai sebagai draf awal deliverable UTS mereka.
:::
