...
1. **Anatomi tabel** dan dasar-dasar spreadsheet
2. **Unit analisis** dan prinsip *tidy data* ([Wickham, 2014](https://doi.org/10.18637/jss.v059.i10))
3. Bentuk **panjang (*long*) vs. lebar (*wide*)**
4. Studi kasus: menata dataset ***Big Five Personality Test***
5. Menyusun tabel ringkasan: **distribusi frekuensi** dan **tabel kontingensi**
6. Menyajikan tabel dengan baik
...
**Fungsi** adalah kalkulasi siap pakai bawaan dari *spreadsheet* — lebih ringkas daripada menulis `=B3+B4+B5+B6+B7+B8` secara manual ([Open Knowledge Foundation, 2021](libs/okf.pdf)).

::: {.callout-note}
### Kenapa ini relevan?
Skor total skala kepribadian yang akan kita pakai hari ini (mis. skor *Extraversion*) pada dasarnya adalah `=AVERAGE()` dari 10 item. Formula ini akan Anda pakai berkali-kali sepanjang semester.
:::
...
2. **Satu baris mewakili satu observasi.**
...
* Satu baris = satu observasi (satu unit pada satu waktu). Untuk data **pengukuran berulang**, bentuk inilah yang biasanya *tidy*.
:::
::: {.column width="50%"}
***Wide form* (lebar)**
...
*Wide format* tetap berguna untuk **presentasi** (mis. tabel ringkasan di naskah publikasi). Yang penting: simpan data dalam bentuk *tidy* untuk **analisis** (untuk pengukuran berulang, biasanya *long*), dan konversi ke *wide* kalau perlu untuk **penyajian** (Pertemuan 7).
:::
...
* Tiap **kolom** = satu variabel: 50 kolom item kepribadian (`E1`–`E10`, `N1`–`N10`, `A1`–`A10`, `C1`–`C10`, `O1`–`O10`), plus kolom demografis (`age`, `gender`, dst.) dan `country` (ditentukan dari lokasi teknis/IP partisipan, bukan diisi sendiri).
* Setiap sel berisi **satu nilai**: respons 1–5 pada skala Likert, atau satu nilai demografis.
:::

::: {.fragment}
Apakah struktur ini *tidy*? **Ya** — satu variabel per kolom, satu partisipan per baris, satu tabel untuk satu jenis observasi (respons survei). Bentuknya memang lebar (50 kolom item), tapi tetap *tidy* karena tiap item adalah variabel yang berbeda: *tidy* ≠ *long*.
:::
...
::: {.fragment}
Di `data.csv`, item `E1`–`E10` ada di kolom H–Q, jadi untuk partisipan di baris 2: `=AVERAGE(H2:Q2)` — inilah kenapa formula `AVERAGE()` yang kita bahas di awal tadi sangat penting. Rumus ini baru tepat setelah item *reverse-keyed* (`E2`, `E4`, `E6`, `E8`, `E10`) dibalik skornya (6 − skor) dan nilai `0` (= tidak dijawab) dikosongkan. Kita bahas saat membersihkan data di Pertemuan 4.
:::
...
Tabel kiri cocok untuk **satu** slide laporan (ringkasan seluruh sampel). Tabel kanan menyimpan skor **per partisipan**, per *trait* (bentuk *long*; satu kolom per *trait* juga sama *tidy*-nya) — inilah bentuk yang bisa difilter, dibandingkan antar kelompok, dan divisualisasikan.
:::
...
<small>Angka hipotetis. Persentase di sini dihitung **per baris** (persentase kategori Extraversion dalam tiap gender).</small>
...
*"15 kasus burnout per 100 mahasiswa per semester"* (hipotetis).
:::
:::
...
## Asosiasi lewat tabel kontingensi (*overview* singkat)

Tabel kontingensi juga bisa dipakai menghitung ukuran asosiasi antar variabel kategorikal, misalnya *odds ratio* (bukan korelasi, melainkan rasio peluang), *Cramer's V*, atau *tetrachoric correlation* (khusus dua variabel dikotomis) — tapi ketiganya **tidak** dibahas mendalam di mata kuliah ini. Cukup ketahui bahwa tabel kontingensi juga berfungsi sebagai alat analisis.
...
::: {.incremental}
* Gaya APA umumnya **menghindari garis vertikal** — cukup garis horizontal di bagian atas tabel, di bawah baris judul kolom, dan di bagian bawah tabel.
...
* Gunakan font yang punya **tabular figures** (tiap digit selebar sama sehingga angka sejajar dalam kolom) agar deretan angka lebih mudah dibandingkan secara visual. Ini tidak sama dengan font *monospace* seperti Courier, yang kurang cocok untuk naskah.
:::
...
* Prinsip ***tidy data*** ([Wickham, 2014](https://doi.org/10.18637/jss.v059.i10)): satu variabel per kolom, satu observasi per baris, satu tabel per observational unit.
* Bentuk *tidy* untuk analisis (untuk pengukuran berulang biasanya **long format**), **wide format** untuk presentasi.
* Dataset ***Big Five*** kita sudah *tidy* di level item, tapi perlu di*reshape* ke level skala (`AVERAGE()`, setelah item *reverse-keyed* dibalik) untuk dianalisis sebagai skor kepribadian.
