...
*"Semua data pada dasarnya jelek, kita hanya perlu tahu seberapa jelek."* (Open Knowledge Foundation, 2021)
...
Menurut Open Knowledge Foundation (2021), ada tiga kriteria:
...
4. ***Common sense check*** — kemampuan mengenali pola yang aneh atau nilai yang "tidak masuk akal", berdasarkan pengetahuan Anda tentang topiknya (Open Knowledge Foundation, 2021).
:::
...
* ***Wild codes* (nilai di luar rentang)** — nilai yang berada di luar rentang yang valid. 
  + Contoh: skor `9` pada skala Likert 1–6; usia partisipan `350` tahun; skor IQ `250`; kode jenis kelamin `M` atau `Pria` padahal *codebook* hanya mengizinkan `1`/`2`.
...
* ***Implausible values* (nilai yang tidak masuk akal)** — valid secara teknis (dalam rentang, konsisten antarkolom), tapi tidak masuk akal secara substantif. 
  + Contoh: durasi tidur `20` jam per malam; penghasilan rumah tangga Rp1 triliun/bulan.
:::
...
* Deteksi cepat: urutkan kolom atau buat *boxplot* untuk melihat ujung distribusinya, atau pakai *Z-score*, yang mana, nilai **di luar *mean* ± 2SD patut dicurigai**, **di luar ± 3SD patut diperiksa lebih lanjut**. Karena *mean* dan SD sendiri terpengaruh *outlier*, batas *median* ± 2,5 × MAD (*median absolute deviation*) lebih kokoh ([Leys et al., 2013](https://doi.org/10.1016/j.jesp.2013.03.013)).
:::
...
Tidak semua *missing value* dapat mengganggu proses analisis data. Polanya perlu diamati untuk menentukan seberapa aman ketika Anda mengabaikannya begitu saja. Berikut jenisnya ([van Buuren, 2018](https://stefvanbuuren.name/fimd/sec-MCAR.html)):
...
* ***Missing at random* (MAR)** — probabilitas hilangnya data berkaitan dengan variabel *lain* yang terekam di dataset, bukan dengan data itu sendiri. 
  + Contoh: mahasiswa laki-laki lebih jarang menjawab pertanyaan tentang "kesulitan mengekspresikan emosi", tapi di dalam kelompok jenis kelamin yang sama, kemungkinan tidak menjawab tidak bergantung pada skornya. Hilangnya berkaitan dengan jenis kelamin (terekam), bukan skornya sendiri.
* ***Not missing at random* (NMAR)** — van Buuren (2018) menyebutnya *missing not at random* (MNAR); probabilitas hilangnya data berkaitan langsung dengan data yang tidak terekam itu sendiri. 
  + Contoh: mahasiswa yang paling sering menyontek justru paling mungkin tidak menjawab pertanyaan "seberapa sering Anda menyontek?" sehingga paling berisiko menghasilkan bias.
:::
...
Dataset psikometrik yang dikumpulkan **secara daring dan terbuka** (siapa saja bisa mengisi), seperti dataset contoh kita, cenderung punya masalah berikut:
...
* **Kode negara yang tidak standar** — di dataset asli ada nilai `(nu` dan kode non-negara `A1`/`A2`; di *file* latihan nanti, campuran nama negara, kode ISO, dan nilai kosong.
:::
...
::: {.callout-note appearance="simple"}
Istilah **Editing** di tahap 3 ini adalah payung besar untuk tiga aktivitas di *slide* berikutnya (*tidying*, *editing* konten, *consolidation*). Jangan tertukar dengan "data *editing*" sebagai salah satu dari ketiganya. Tiga tahap ini mengikuti [Van den Broeck et al. (2005)](https://doi.org/10.1371/journal.pmed.0020267); pembagian isi tahap *Editing* diadaptasi dari Open Knowledge Foundation (2021).
:::
...
::: {.callout-note appearance="simple"}
Norma umum di sains data adalah ilmuwan/praktisi data menghabiskan 80% waktunya bekerja untuk membersihkannya (Open Knowledge Foundation, 2021). Angka ini sebenarnya ungkapan populer; survei terhadap praktisi data melaporkan angka sekitar 45–60%.
:::
...
* Dokumentasi ini menolong ingatan Anda yang terbatas.
...
Unduh potongan kecil dataset *Big Five* berikut — 18 baris respons **N1–N10** (item *Neuroticism*) yang sengaja dibuat "kotor" untuk latihan: [`pertemuan4_dataset_kotor.csv`](../data/pertemuan4_dataset_kotor.csv). Ikuti langkah berikut:
...
Dua dari sepuluh item *Neuroticism* ini ditulis dengan arah terbalik: **N2** ("*I am relaxed most of the time*") dan **N4** ("*I seldom feel blue*", lihat *slide* Studi kasus). Keduanya perlu dibalik (`6 - nilai`) sebelum dihitung skor rata-ratanya dengan `AVERAGE()` — informasi ini **tidak** ada di dalam data itu sendiri, hanya di kunci skoring IPIP (`codebook.txt` tidak menandainya).

File `data/pertemuan4_dataset_kotor.csv` sudah disiapkan (18 baris: 17 responden sungguhan dari dataset *Big Five* kita, 4 di antaranya nilai N-nya dimodifikasi (`P07`, `P12`, `P14`, `P15`), + 1 baris duplikat (`P03`); kolom demografis sengaja dikotori). Kunci jawaban untuk dosen:
...
* **Kode negara campur (`negara`)** — kombinasi kode ISO (`PH`, `SG`, `CA`, `US`, `DK`, `GB`, `IN`), nama lengkap (`Trinidad and Tobago`, `Pakistan`, `Greece`, `Bangladesh`, `United Kingdom`, `Jamaica`, `South Africa`), dan satu nilai kosong (`P09`).
...
* **Item *reverse-scored* belum dibalik** — N2 dan N4 butuh `6 - nilai` sebelum di-*AVERAGE()*-kan; ini **tidak** terlihat dari data mentahnya saja, harus dicek ke kunci skoring IPIP (lihat catatan di atas). Ini poin diskusi utama untuk langkah 4 aktivitas.
